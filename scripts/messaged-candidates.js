...
function handleFilterSelection() {
    ...
    if (loggedInUserObj.fields["Candidates interested in"]) {
        usersMessaged = loggedInUserObj.fields["Candidates interested in"].map(candidate => {
...
function fetchProfiles(filter) {
    ...
    fetch(APIURL, requestOptions)
        ...
        // .then(() => {
        //     userbase.forEach(message => {
        //         fetch(API + "Users&id=" + message.id, requestOptions)
        //         .then(response => response.json())
        //         .then(response => {
        //             message.
        //         })
...
function getLoggedInUserData(userId) {
    ...
    fetch(API + "&id=" + userId + '&cacheTime=0', requestOptions)
        ...
        .then(() => {
            ...
            if (loggedInUserObj.fields["Introductions sent"]) {
                 usersMessaged = [loggedInUserObj.fields["Introductions sent"]].map(message => {
                    return `{Airtable ID}="${message}"`
                }).join(',')
                const filteredOptions = `IF(OR(${usersMessaged}),"true")`
                
                const filterEncode = "&filterByFormula=" + encodeURI(filteredOptions)  
                console.log(usersMessaged, filterEncode, filteredOptions)
